...
run "a_generated_json_entry_joins_the_same_blob_as_the_passed_in_keys" {
  command = plan
  ...
  assert {
    condition     = local.json_generate_keys == toset(["app"])
    error_message = "A secret with json_generate must be written by the json_generate resource, not the plain one, because only that path merges the generated keys into the blob."
  }

  assert {
    condition     = local.managed_keys == toset([])
    error_message = "A json_generate secret must not also get a plain version resource, or two resources would fight over the same secret."
  }

  assert {
    condition     = local.static_json_maps["app"] == { SECRET_KEY = "not-a-real-key" }
    error_message = "The passed-in keys must survive into the blob untouched, since json_generate only adds keys rather than replacing the map."
  }

  assert {
    condition     = keys(local.json_generate_bytes_entries) == ["app.mfa_master_key"]
    error_message = "A bytes32-base64 entry must be generated by random_bytes, which is the only generator that yields 32 raw bytes rather than a character string."
  }
}

run "one_secret_can_mix_both_generator_formats" {
  command = plan
  ...
  variables {
    secrets = {
      app = {
        json_generate = {
          mfa_master_key = { format = "bytes32-base64" }
          api_key        = { format = "password", length = 40 }
        }
      }
    }
  }
  ...
  assert {
    condition     = keys(local.json_generate_bytes_entries) == ["app.mfa_master_key"]
    error_message = "Only the bytes entry belongs to the random_bytes generator."
  }

  assert {
    condition     = keys(local.json_generate_password_entries) == ["app.api_key"]
    error_message = "Only the password entry belongs to the random_password generator."
  }
}

run "a_kept_entry_is_read_back_rather_than_regenerated" {
  command = plan
  ...
  assert {
    ...
    error_message = "keep must mark the entry as carried forward, so a later write of the blob reuses the stored value instead of minting a new one."
  }

  assert {
    condition     = local.json_generate_carry_keys == toset(["app"])
    error_message = "A secret holding any kept entry must read its current version, because that read is where the existing value comes from."
  }
}

run "a_secret_with_no_kept_entry_never_reads_itself_back" {
  command = plan
  ...
  assert {
    ...
    error_message = "With nothing to carry forward the module must not read the secret at all, so the first apply of a brand new secret does not fail on a version that does not exist yet."
  }
}

run "an_unknown_generate_format_is_rejected" {
  command = plan
  ...
  variables {
    secrets = {
      app = {
        json_generate = {
          mfa_master_key = { format = "bytes64-hex" }
        }
      }
    }
  }
...
run "a_key_cannot_be_both_passed_in_and_generated" {
  command = plan

  variables {
    secrets = {
      app = {
        json          = { mfa_master_key = "not-a-real-key" }
        json_generate = { mfa_master_key = { format = "bytes32-base64" } }
      }
    }
  }
...
run "json_generate_cannot_be_combined_with_a_plain_value" {
  command = plan
