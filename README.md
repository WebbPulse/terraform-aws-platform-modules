...
| [`modules/api-alarms`](modules/api-alarms/) | SNS topic with email subscriptions plus CloudWatch alarms for Lambda errors and throttles, either per function or summed across a function per domain, HTTP API 5xx and integration latency, and DynamoDB throttles either as one alarm per environment or one per table. |
