...
| [`modules/app-secrets`](modules/app-secrets/) | A map of Secrets Manager secrets in five shapes (generated, given, JSON, placeholder, empty), written write-only so no value enters state, with opt-in operator-owned JSON keys that survive a version bump and a ready-made read policy for the app role. |
| [`modules/operator-config`](modules/operator-config/) | One free Standard SSM `String` parameter per app and environment holding an operator-owned JSON object of private non-secret config, seeded `{}` with `ignore_changes` and exposed decoded, for inputs such as `ses_verified_recipients`. |
...
| [`modules/step-functions`](modules/step-functions/) | A Standard state machine from a caller-supplied definition with substitutions, its execution role with a caller-supplied statement list plus the vended logging grant, a log group with retention, log level and execution-data toggles, optional X-Ray tracing, and a ready-made policy for a caller that starts executions and completes task tokens. |
| [`modules/sqs-queue`](modules/sqs-queue/) | A queue and the dead letter queue behind it: a redrive policy with a max receive count, a visibility timeout checked against the consuming Lambda's own timeout, SSE-SQS by default or a KMS key, optional FIFO, a redrive allow policy naming this queue as the only source, and an optional queue policy granting a list of producer roles `sqs:SendMessage`. |
| [`modules/transaction-search`](modules/transaction-search/) | X-Ray Transaction Search for one account and region: the CloudWatch Logs resource policy X-Ray puts span events under, the CloudWatchLogs trace segment destination, the Default indexing rule, and an optional adoption of the reserved `aws/spans` log group so its retention is managed. |
| [`modules/ses-identity`](modules/ses-identity/) | A product's transactional SES v2 setup: a configuration set, one sending identity as a domain with Easy DKIM or a single address, an optional custom MAIL FROM, feedback attributes, an optional SNS event destination, the DKIM and DMARC records, and the explicitly listed recipient identities a sandboxed account needs. Authors resources only; sandbox and production access stay out of band. |
| [`modules/ecs-fargate`](modules/ecs-fargate/) | A cluster with Container Insights off and a map of Fargate task definitions for on-demand tasks: arm64 by default, a log group and task role per task, one shared execution role whose secret read grants are derived from the task map, and a ready-made `ecs:RunTask` policy with the `iam:PassRole` that call needs. No services, no load balancers, no scheduling. |
